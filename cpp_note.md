...
- [四、类与对象](#四类与对象)
  ...
  - [7.结构体与联合体](#7结构体与联合体)
  - [8.枚举类](#8枚举类)
- [五、数据的共享与保护](#五数据的共享与保护)
  - [1.标识符的作用域与可见性](#1标识符的作用域与可见性)
  - [2.对象的生存期](#2对象的生存期)
  - [3.类的静态成员](#3类的静态成员)
  - [4.类的友元](#4类的友元)
  - [5.共享数据的保护](#5共享数据的保护)
  - [6.多文件结构和预编译命令](#6多文件结构和预编译命令)
- [六、数组、指针与字符串(一)](#六数组指针与字符串一)
  - [1.数组的定义与初始化](#1数组的定义与初始化)
  - [2.数组作为函数的参数](#2数组作为函数的参数)
  - [3.对象数组](#3对象数组)
  - [4.基于范围的for循环](#4基于范围的for循环)
  - [5.指针的定义和运算](#5指针的定义和运算)
- [六、数组、指针与字符串(二)](#六数组指针与字符串二)
  - [1.指针与数组](#1指针与数组)
  - [2.指针与函数](#2指针与函数)
  - [3.对象指针](#3对象指针)
  - [4.动态内存分配](#4动态内存分配)
  - [5.智能指针](#5智能指针)
  - [6.vector对象](#6vector对象)
  - [7.对象复制与移动](#7对象复制与移动)
  - [8.字符串](#8字符串)
...
- 作用域分类
  ...
  - 文件作用域（静态作用域）
    ...
    - 可见性
      ...
      - 如果标识在某处可见，就可以在该处引用标识符。
  - 命名空间作用域
    - 可见性
      - 如果某个标识符在外层中声明，且在内层中没有同一标识符的声明，则该标识符在内层可见。
      - 对于两个嵌套的作用域，如果在内层作用域内声明了与外层作用域同名的标识符，则外层作用域的标识符在内层不可见。
## 2.对象的生存期
...
- 友元类
  ...
  - 声明语法：将友元类名在另一个类中使用`friend`修饰声明。
...
- 数组的定义与使用
  ...
  - 数组的定义
    - 类型说明符 数组名[常量表达式][常量表达式]......;数组名的构成方法与一般变量名相同。
  - 数组元素的使用
    - 数组必须先定义，后使用。
    - 可以逐个引用数组元素。
- 数组的储存与初始化
  - 一维数组的存储
    - *数组元素*在内存中顺次存放，它们的*地址是连续的*。元素间物理地址上的相邻，对应着逻辑次序上的相邻。
  - 一维数组的初始化
    - 列出全部元素的初始值
    - 可以只给一部分元素指定初值
...
- 数组元素的构造和析构
  - 构造数组时，元素所属的类未声明构造函数时，则采用默认构造函数。
...
- 动态分配与释放内存
  ...
  `new 类型名T（初始化参数列表）`
    ...
    - 结果值：`T`类型的指针，指向新分配的内存；失败：抛出异常。
  - 释放内存操作符`delete` `delete 指针p`
...
- `vector`对象的使用
  - 对数组元素的引用——与普通数组具有相同形式：`vector对象名[下标表达式]`——`vector`数组对象名不表示数组首地址
  - 获得数组长度——用`size`函数——`vector对象名.size()`
## 7.对象复制与移动
- 浅层复制和深层复制
  - 浅层复制
    - 实现对象间数据元素的一一对应复制。
